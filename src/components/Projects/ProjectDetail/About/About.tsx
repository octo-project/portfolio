    ...
    <div className="projectDetails">
      <div style={{ display: "none" }}>
        <h5 style={{ fontSize: "15px", color: "black", opacity: 0.2 }}>
          <u>
            <a href={about.demoUrl} target="_blank" rel="noopener noreferrer">
              {formatText("seeDemo")}
            </a>
          </u>
        </h5>
        <h5 style={{ fontSize: "15px", color: "black", opacity: 0.2 }}>
          <u>
            <a
              href={about.gitHubRepository}
              target="_blank"
              rel="noopener noreferrer"
            >
              {formatText("repository")}
            </a>
          </u>
        </h5>
      </div>
      ...
      <ul>
        {(about?.missions || []).map((mission: any, index: number) => {
          return <li key={index}>{formatText(mission)}</li>;
        })}
      </ul>
      <h5>{formatText("Functionnality")} :</h5>
      <div className="technoGrid">
        {about.functionnalities.map((item: any, index: number) => {
          return (
            <span className="functionnalityCard purpleBg animate" key={index}>
              {formatText(item)}
            </span>
          );
        })}
